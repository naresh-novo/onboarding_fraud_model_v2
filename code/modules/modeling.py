...
class ModelMetric:
    ...
    def model_metrics(self, y_pred: np.ndarray, y_true: np.ndarray, y_prob: np.ndarray,tag):
        ...
        output_dict = {'df_tag':tag,'accuracy_score': ac, 'FPR':np.round(fpr[1],2) , 'TPR':np.round(tpr[1],2), 'AUC': np.round(auc(fpr, tpr), 5), 'Sensitivity':np.round(tp/(tp+fn),2), 'Specificity':np.round(tn/(tn+fp),2), 'KS Stat':ks_2samp(y_pred, y_true)}
    ...
    def probability_binned(self,model: object, X: pd.DataFrame, 
        ...
        X['volume_binned'] = pd.qcut(X['proba'],q=10)
    ...
    def rank_ordering_log_reg(self, model, x_train, y_train, x_test, y_test, feature_list):
        ...
        rnk_df_all = self.probability_binned(model,df_all[np.append([feature_list], ['fpd_plus_3'])], 'fpd_plus_3', aggregate_func='rate')
        rnk_df_test = self.probability_binned(model,df_test[np.append([feature_list], ['fpd_plus_3'])], 'fpd_plus_3', aggregate_func='rate')
        rnk_df_train = self.probability_binned(model,df_train[np.append([feature_list], ['fpd_plus_3'])], 'fpd_plus_3', aggregate_func='rate')
